...
const META_PURCHASE_BROWSER_FLUSH_DELAY_MS = 350;
...
function waitForMetaPurchaseDispatch() {
  return new Promise((resolve) => {
    setTimeout(resolve, META_PURCHASE_BROWSER_FLUSH_DELAY_MS);
  });
}

export async function trackMetaPurchaseEvents(events = []) {
  let dispatchedEvents = 0;

  events.forEach((event) => {
    if (!event?.eventId || !event?.customData) return;

    const eventId = trackMetaBrowserEvent(META_EVENTS.PURCHASE, event.customData, {
  ...
  if (dispatchedEvents > 0) {
    await waitForMetaPurchaseDispatch();
  }
